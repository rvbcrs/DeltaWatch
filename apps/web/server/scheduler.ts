...
const isBrowserError = (errorMessage: string): boolean => {
    const browserErrorPatterns = [
        ...
        // HTTP 5xx / Infrastructure errors (should not count as monitor failures)
        'Gateway Timeout',
        'Bad Gateway',
        '504',
        '502',
        ...
        'Overall check timeout exceeded',
        'page.goto: Timeout'
    ];
...
export const INTERVAL_MINUTES: Record<string, number> = {
    ...
    '24h': 1440,
    '1w': 10080,
    // Compat for web editor cron strings
    '*/1 * * * *': 1,
    '*/5 * * * *': 5,
    '*/15 * * * *': 15,
    '*/30 * * * *': 30, // Normalized
...
let lastWatchdogNotification = 0; // Rate-limit watchdog emails (1 per hour max)
const WATCHDOG_NOTIFICATION_COOLDOWN = 60 * 60 * 1000; // 1 hour in ms
...
async function checkSingleMonitor(monitor: Monitor, context: BrowserContext | null = null): Promise<void> {
    ...
    try {
        ...
        // Wait for selector
        let selectorVisibilityFailed = false;
        if (monitor.selector && monitor.type === 'text') {
            try {
                await page.waitForSelector(monitor.selector, { state: 'visible', timeout: 10000 });
            } catch (e) {
                try {
                    ...
                    // Mark for potential healing later, but don't trigger yet
                    selectorVisibilityFailed = true;
                    console.log(`[${monitorName}] Selector visibility check failed. Will evaluate after content extraction.`);
                }
            }
        }
        ...
        if (monitor.selector) {
            ...
            if (text === null) {
                ...
                // NOW trigger AI Self-Healing (only if content extraction truly failed)
                if (selectorVisibilityFailed && !monitor.suggested_selector && (!httpStatus || httpStatus < 400)) {
                    console.log(`[${monitorName}] Content extraction failed. Triggering AI Self-Healing...`);
                    try {
                        if (!page.isClosed()) {
        ...
        if (monitor.price_detection_enabled) {
            ...
            const priceResult = await extractPrice(page, text || undefined);
            
            if (priceResult) {
                detectedPrice = priceResult.price;
                detectedCurrency = priceResult.currency;
                console.log(`[${monitorName}] Detected price: ${formatPrice(detectedPrice, detectedCurrency)} (source: ${priceResult.source})`);
                
                // Check if price changed from last detected
                if (monitor.detected_price && monitor.detected_price !== detectedPrice) {
                    priceChanged = true;
                    const oldPrice = formatPrice(monitor.detected_price, detectedCurrency);
                    const newPrice = formatPrice(detectedPrice, detectedCurrency);
        ...
        if (textChange || visualChange || priceChanged || priceAlertTriggered || stockAlertTriggered) {
            ...
            if (priceAlertTriggered) changeMsg += priceAlertMessage + " ";
                ...
                changeMsg += `${direction} Price: ${formatPrice(monitor.detected_price, detectedCurrency)} → ${formatPrice(detectedPrice, detectedCurrency)}. `;
            }
        ...
        if (changed) {
            if (monitor.type === 'visual') {
                db.run(
                    `UPDATE monitors SET last_check = ?, last_value = ?, last_screenshot = ?, last_change = ?, detected_price = ?, detected_currency = ?, unread_count = unread_count + 1, consecutive_failures = 0 WHERE id = ?`,
                    [nowStr, text, screenshotPath, nowStr, detectedPrice, detectedCurrency, monitor.id],
                    (err: Error | null) => { if (err) console.error("Update Error:", err); }
                );
            } else {
                db.run(
                    `UPDATE monitors SET last_check = ?, last_value = ?, last_change = ?, detected_price = ?, detected_currency = ?, unread_count = unread_count + 1, consecutive_failures = 0 WHERE id = ?`,
                    [nowStr, text, nowStr, detectedPrice, detectedCurrency, monitor.id],
                    (err: Error | null) => { if (err) console.error("Update Error:", err); }
                );
            ...
            if (monitor.type === 'visual') {
                db.run(
                    `UPDATE monitors SET last_check = ?, last_screenshot = ?, last_value = ?, detected_price = ?, detected_currency = ?, consecutive_failures = 0 WHERE id = ?`,
                    [nowStr, screenshotPath, text, detectedPrice, detectedCurrency, monitor.id],
                    (err: Error | null) => { if (err) console.error("Update Error:", err); }
                );
            } else {
                // For text monitors, always update last_value (needed for first run baseline)
                db.run(
                    `UPDATE monitors SET last_check = ?, last_value = ?, detected_price = ?, detected_currency = ?, consecutive_failures = 0 WHERE id = ?`,
                    [nowStr, text, detectedPrice, detectedCurrency, monitor.id],
                    (err: Error | null) => { if (err) console.error("Update Error:", err); }
                );
                fs.unlink(screenshotPath, (delErr) => { if (delErr) console.error("Error deleting unused screenshot:", delErr) });
            }
        ...
        if (!isBrowserError(error.message)) {
            ...
            console.log(`[${monitorName}] Browser/infrastructure error - skipping history log and failure count`);
                ...
                if (totalActiveMonitors > 0 && cooldownPercentage >= threshold) {
                    ...
                    db.run("UPDATE monitors SET consecutive_failures = 0", [], (updateErr: Error | null) => {
                        if (updateErr) {
                            ...
                            // Send notification about recovery (rate-limited to 1 per hour)
                            const notifyNow = Date.now();
                            if (notifyNow - lastWatchdogNotification > WATCHDOG_NOTIFICATION_COOLDOWN) {
                                lastWatchdogNotification = notifyNow;
                                sendNotification(
                                ...
                                console.log('[WATCHDOG] Notification skipped (rate-limited, 1 per hour max)');
                            }
                        }
                        resolve();
                    });
