...
app.post('/monitors', auth.authenticateToken, (req, res) => {
    ...
    const userId = req.user?.userId;
    db_1.default.run(`INSERT INTO monitors (user_id, url, selector, selector_text, interval, type, name, notify_config, ai_prompt, tags, keywords, ai_only_visual, group_id, price_detection_enabled, price_threshold_min, price_threshold_max) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [userId, url, selector, selector_text || '', interval || '30m', type || 'text', name, JSON.stringify(notify_config), ai_prompt, JSON.stringify(tags), JSON.stringify(keywords), ai_only_visual ? 1 : 0, group_id || null, price_detection_enabled || 0, price_threshold_min || null, price_threshold_max || null], function (err) {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
